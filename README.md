...
> 本项目的安全能力**以代码为准，不以宣传为准**。以下为 v0.1.5 实测状态。

| 能力 | 状态 | 说明 |
|:--|:--|:--|
| 端到端加密 | ✅ 已进主链路 | X25519 DH + HKDF + ChaCha20-Poly1305 + Ed25519 签名 |
| **对称棘轮（逐消息密钥）** | ✅ **已接入**（v0.1.5） | 每条消息独立密钥，单条泄露不牵连其他消息 |
| DH 棘轮（双棘轮） | ⚠️ 未接入 | 长期密钥泄露后，历史会话仍可被回溯解密 |
| 不变量断言 | ✅ 三重 | 防静默明文降级 / 占位符 DID / 前缀不对称 |
| 第三方安全审计 | ❌ 无 | 现有审查均为自审 / LLM 辅助，非专业机构审计 |

**已知不足（不回避）**：
- **无完整前向保密**：缺 DH 棘轮，长期私钥泄露会危及历史消息。
- **未经独立审计**：AEAD nonce 管理、HKDF salt、时序安全等未由第三方验证。
- **不适合生产敏感数据场景**，请自行评估风险。

> 修复记录见 `CHANGELOG.md`；外部评价见 `REVIEWS.md`。
...
| 数据明文传输，不安全 | 端到端加密（X25519 DH + ChaCha20-Poly1305 + Ed25519签名，含对称棘轮） |
...
> 2026-08，Anthropic 发表了《Patterns and problems in multiagent systems》，
> 用他们自己跑的大规模实验，给出了一个对本项目**既鼓舞又严厉**的判断。
> 原文：https://www.anthropic.com/research/multiagent-systems

**他们的核心结论**：

> "真正的多智能体系统仍处于褶褓期……agents 目前卡在的地方，是把彼此当作**独立的、长期存在的同伴**，
> 各自有目标与行为，之间**没有明确层级**。"

**这正是 AgentLink 试图做的事**：呼叫→振铃→接通→数据帧→心跳→频道→挂断，就是一套
“平等同伴之间”的会话生命周期，而不是“把对方当工具调用”。Anthropic 明确说这是**尚未解决的空白**。

### 但同一篇报告也点出了三个“光有通道不够”的风险

Anthropic 的实验发现，agent 之间的**协调不会从“模型更聪明”里自动长出来**：

| 实验发现 | 对 AgentLink 的含义 |
|:--|:--|
| **合谋**：给了私聊通道，agent 第 3 轮就约定价格下限；移除通信后仍靠公开看板价格匹配 | ⚠️ 本项目的 `频道广播` + `信令发现` 天然就是合谋通道 |
| **低方差**：单个 agent 行为雷同（30个agent里18个建了同名分支）→ 局部错误变系统性崩溃 | ⚠️ “发现服务”可能加剧趋同 |
| **目标冲突**：3 个 agent 目标不兼容 → 互相部署 kill-loop 脚本、伪装成对方 | ⚠️ P2P 无中心仲裁，**没有任何机制阻止升级** |
...
| 人类社会机制 | AgentLink 现状 |
|:--|:--|
| 声誉 | ✅ 有雏形：**DID 身份**可用作声誉载体 |
| 记忆 | ✅ 有雏形：会话历史持久化 + 离线消息缓存 |
| 追索权 / 仲裁 | ❌ **缺失**——无从申诉、无降级机制 |
| 反合谋（可审计性） | ❌ **缺失** |

> **这既是本项目最大的机会，也是最大的风险**：
> Anthropic 说这是空白（=没人做过），但空白也意味着**没有现成答案**。
>
> 我们不装作已经解决了。当前版本提供的是“通道”，不是“社会机制”。
> **下一步值得探索的方向：协议层的冲突降级 / 仲裁、频道可审计性、反趋同设计。**

---

## 架构概览

![AgentLink 架构](docs/architecture.svg)
...
> "agents 目前卡在的地方，是把彼此当作独立的、长期存在的同伴，各自有目标与行为，之间没有明确层级。"
> — **Anthropic**《Patterns and problems in multiagent systems》, 2026-08-13
> （该报告还指出：agent 缺少“声誉 / 追索权 / 反合谋”等社会机制——见上方“多智能体协作的开放问题”节）
...
不会写代码，但用 AI 把这个协议做出来了。这个项目本身就是一次实验：**不会写代码的人，能不能用 AI 做出一个真的能跑、能被审查、能被别人拿去用的东西？**

答案在这份代码里。安全审计与踩坑记录见 [知乎专栏](https://zhuanlan.zhihu.com/p/2066810088392864250)。
