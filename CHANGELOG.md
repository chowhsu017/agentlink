...
> 主题：**把"死代码"变成"活能力"**。修复前向保密的声明与实现不一致问题。

### 🐛 修复（关键）

- **对称棘轮接入主链路**：`SessionCipher.ratchet_encrypt/ratchet_decrypt` 虽早已实现，但**零调用点（死代码）**——
  `SecureSessionManager.encrypt_payload` 实际调用的是普通 `cipher.encrypt()`，**没有任何棘轮**。
  本次将对称棘轮接入加密/解密路径：
  - 每条消息用 HKDF 派生独立密钥 → **单条消息密钥泄露不牵连其他消息**。
  - 解密侧先试棘轮、失败回退普通解密（兼容旧对端/旧数据）。

### 🧪 新增测试

- `TestRatchet`：棘轮往返 / 逐消息密钥唯一性 / **前向安全验证**（旧密钥解不了新消息）。
- 修正 `test_channel_key`：原断言假设"非 E2EE 频道也返回密钥"，与现行为（返回 None 防假密钥）不符。

### 🔧 工程

- **新增 CI**（`.github/workflows/tests.yml`）：Python 3.10/3.11/3.12 矩阵跑 pytest + 不变量断言测试。

### 📖 文档

- README 新增「安全现状（诚实说明）」表：明确列出**已具备**（E2EE、对称棘轮、不变量断言）与
  **已知不足**（无 DH 棘轮 = 无完整前向保密、未经第三方审计）。

### ⚠️ 诚实说明

本次修复解决的是**对称棘轮**（消息级前向安全）。**DH 棘轮（双棘轮）仍未接入**，
故长期私钥泄露后可回溯解密历史会话的风险**依然存在**——已如实写入 README，不掩饰。
